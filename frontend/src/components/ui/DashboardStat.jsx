...
const DashboardStat = ({ icon, label, value, color = colors.text, bg = colors.bgMuted, sublabel, sx = {} }) => (
  <Card sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', minHeight: 120, ...sx }}>
    ...
    <Typography sx={{ color: colors.text, fontWeight: 600, fontSize: 15 }}>{label}</Typography>
    <Typography sx={{ fontWeight: 900, fontSize: 32, color: color }}>{value}</Typography>
    {sublabel && <Typography sx={{ color: colors.text, fontWeight: 700, fontSize: 13 }}>{sublabel}</Typography>}
  </Card>
);
